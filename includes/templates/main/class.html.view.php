		...
		public static function comment_form($name, $email, $content, $error_name, $error_email, $error_content, $error_captcha, $form_side_image, $n1, $n2, $result){
			...
			echo '<div id="respond">'.
					...
					'<div id="form_side_image">'.
						'<img src="images/'.$form_side_image.'_comment.png" alt="" />'.
					'</div>'.
				 '</div>';
		...
		public static function video($name, $author, $link, $embed_code, $description, $date){
			...
			if(VSession::html5()){
				...
				echo '<section class="video">'.
					 	...
					 	'</video>'.
					 	'<details>'.
					 		'<summary>"'.$name.'" is a video made by '.$author.' (uploaded the <time datetime="'.date(DATE_ATOM, strtotime($date)).'" pubdate>'.date('M d, Y @ H:i', strtotime($date)).'</time>)</summary>'.
					 		'<p>'.
					 			$description.
					 		'</p>'.
					 	'</details>'.
