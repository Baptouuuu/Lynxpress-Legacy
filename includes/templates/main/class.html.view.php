		...
		*/
		...
		public static function listing_article($title, $permalink, $categories, $date, $author, $content){
			...
			if(VSession::renderer() == 'mobile'){
				...
				if(Vsession::html5())
					echo '<div class="triangle left"></div><div class="triangle right"></div>';
					
				if(Vsession::html5())
					echo '<div class="listing_icon"><img src="images/icon_title.png" alt="" /></div>';
				
				if(Vsession::html5())
					echo '<aside>';
				else
					echo '<div class="listing_ref">';
				
				echo		'published the '.date('M d', strtotime($date)).'<br/>'.
							'by '.$author.'<br/>'.
							'categories: '.implode('| ', $categories).'<br/>';
							
				if(Vsession::html5())
					echo '</aside><article>';
				else
					echo '</div><div class="listing_news">';
		...
		*/
		...
		public static function comment_form($name, $email, $content, $error_name, $error_email, $error_content, $error_captcha, $form_side_image, $n1, $n2, $result){
			...
			echo '<div id="respond">'.
					...
					'</form>'.
					'<div id="form_side_image">';
					
					if($form_side_image == 'new')
						echo '<img src="images/new_comment.png" alt="" />';
					elseif($form_side_image == 'persuit')
						echo '<img src="images/persuit_comment.png" alt="" />';
		...
		public static function video($name, $author, $link, $embed_code, $description, $date){
			...
			if(VSession::html5()){
				...
				echo '<section class="video">'.
					 	'<video width="640" src="'.$link.'" controls preload="auto">'.
					 		$embed_code.													//in case the browser doesn't support video tag yet
					 	'</video>'.
					 	'<details>'.
					 		'<summary>"'.$name.'" is a video made by '.$author.' (uploaded the <time datetime="'.$date.'" pubdate>'.date('M d, Y @ H:i', strtotime($date)).'</time>)</summary>'.
					 		'<p>'.
					 			$description.
					 		'</p>'.
					 	'</details>'.
